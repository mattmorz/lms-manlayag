...
class TestGradingPolicy(BaseTestUtils):
	...
	def test_weighted_grade_calculation(self):
		...
		course.append(
			"grading_categories", {"category_name": "Homework", "weight": 40, "number_of_assessments": 5}
		)
		course.append(
			"grading_categories", {"category_name": "Exam", "weight": 60, "number_of_assessments": 1}
		)
					...
					"blocks": [
						{
							"id": "quiz1",
							"type": "quiz",
							"data": {"quiz": quiz.name, "grading_category": "Exam"},
						},
						{
							"id": "assignment1",
							"type": "assignment",
							"data": {"assignment": assignment.name, "grading_category": "Homework"},
						},
					],
					"version": "2.29.0",
				}
	...
	def test_category_assessment_limit(self):
		...
		course.append(
			"grading_categories", {"category_name": "Homework", "weight": 100, "number_of_assessments": 2}
		)
		course.save()
					...
					"blocks": [
						{
							"id": "asg1",
							"type": "assignment",
							"data": {"assignment": a1.name, "grading_category": "Homework"},
						},
						{
							"id": "asg2",
							"type": "assignment",
							"data": {"assignment": a2.name, "grading_category": "Homework"},
						},
					]
				}
			),
							...
							"blocks": [
								{
									"id": "asg3",
									"type": "assignment",
									"data": {"assignment": a3.name, "grading_category": "Homework"},
								}
							]
						}
					),
							...
							"blocks": [
								{
									"id": "quiz1",
									"type": "quiz",
									"data": {"quiz": q.name, "grading_category": "Homework"},
								}
							]
						}
					),
			...
			lesson_nocat.update(
				{
					...
					"course": course.name,
					"content": json.dumps(
						{"blocks": [{"id": "quiz_nocat", "type": "quiz", "data": {"quiz": q.name}}]}
					),
				}
			)
			lesson_nocat.insert()
	...
	def test_late_submission_penalty(self):
		...
		course.append(
			"grading_categories", {"category_name": "Homework", "weight": 100, "number_of_assessments": 5}
		)
		course.save()
	...
	def test_course_import_export_with_grading_policy(self):
		...
		course.append(
			"grading_categories", {"category_name": "Homework", "weight": 30, "number_of_assessments": 3}
		)
		course.append(
			"grading_categories", {"category_name": "Exam", "weight": 70, "number_of_assessments": 1}
		)
		course.append("grading_scale", {"grade": "A", "min_percentage": 90})
		course.append("grading_scale", {"grade": "B", "min_percentage": 80})
		course.save()
		...
		imported_lessons = frappe.get_all(
			"Course Lesson", filters={"course": imported_course.name}, fields=["content", "name"]
		)
	...
	def test_unique_quiz_per_course(self):
		...
		self._create_lesson(
			...
			chapter=chapter.name,
			course=course.name,
			content=json.dumps({"blocks": [{"id": "quiz1", "type": "quiz", "data": {"quiz": quiz.name}}]}),
		)
			...
			lesson2.update(
				{
					...
					"course": course.name,
					"content": json.dumps(
						{"blocks": [{"id": "quiz_dup", "type": "quiz", "data": {"quiz": quiz.name}}]}
					),
				}
			)
			lesson2.insert()
			...
			lesson3.update(
				{
					...
					"content": json.dumps(
						{
							"blocks": [
								{"id": "quiz_a", "type": "quiz", "data": {"quiz": quiz.name}},
								{"id": "quiz_b", "type": "quiz", "data": {"quiz": quiz.name}},
							]
						}
					),
				}
